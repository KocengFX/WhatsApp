...
async function connectToWhatsApp() {
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        if (text) {
            console.log(`📩 Pesan masuk dari ${currentSender}: ${text}`);

            // Simpan pesan terbaru untuk pengirim yang sama
            saveMessage(currentSender, text);

            // Kirim reaction emoji ke pesan pengguna hanya jika pesan masuk
            if (msg.key.fromMe === false) {  // Pastikan pesan ini bukan dari bot itu sendiri
